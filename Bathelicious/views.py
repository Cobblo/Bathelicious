...
from .models import (
    ...
    SmallBanner,
    AboutSettings,
    ReviewVideo,          # ✅ IMPORT REVIEW VIDEO
)
...
def home(request):
    ...
    new_arrivals = Product.objects.filter(
        is_new_arrival=True, is_available=True
    )[:8]
    banners = Banner.objects.all()
    categories = Category.objects.all()
    small_banner = SmallBanner.objects.filter(is_active=True).first()

    # ✅ GET ACTIVE REVIEW VIDEOS IN ORDER
    review_videos = (
        ReviewVideo.objects.filter(is_active=True)
        .order_by("order")[:3]          # show top 3
    )
    ...
    return render(
        ...
        "home.html",
        {
            ...
            "review_videos": review_videos,   # ✅ PASS TO TEMPLATE
        },
    )
...
def bestsellers_view(request):
    ...
    return render(
        request, "store/bestsellers.html", {"bestsellers": bestsellers}
    )
