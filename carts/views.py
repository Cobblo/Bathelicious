...
def cart(request, total=0, quantity=0, cart_items=None):
    ...
    try:
        ...
        if total > 0:
            shipping = _shipping_amount(total)

        offer_discount = _offer_discount_amount(total)
...
def redeem_points(request):
    if request.method == "POST":
        ...
        if actual > 0:
            messages.success(
                request,
                f"₹{actual} redeemed from your points."
            )

        else:
            messages.warning(
                request,
                "Cannot redeem that amount."
            )
...
def apply_coupon(request):
    ...
    messages.success(
        request,
        f"Offer code '{coupon.code}' applied successfully."
    )
